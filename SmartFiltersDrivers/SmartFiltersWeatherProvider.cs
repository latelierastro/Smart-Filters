...
namespace SmartFilters.SmartFiltersDrivers {
    ...
    public class SmartFiltersWeatherProvider : IEquipmentProvider<IWeatherData> {
        private IProfileService profileService;

        [ImportingConstructor]
        public SmartFiltersWeatherProvider(IProfileService profileService) {
            this.profileService = profileService;
        }

        public string Name => "SmartFilters";

        public IList<IWeatherData> GetEquipment() {
            var devices = new List<IWeatherData>();
            devices.Add(new SmartFiltersWeatherDriver("MyUniqueId", "MyDeviceId"));
